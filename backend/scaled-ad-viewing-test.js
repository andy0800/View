// scaled-ad-viewing-test.js
// Scaled-up ad viewing test to measure system performance
...
class ScaledAdViewingTest {
  ...
  async run() {
    try {
      console.log('🚀 Starting Scaled Ad Viewing Test');
      console.log('=' .repeat(60));
      
      // Phase 1: Load existing data
      await this.loadExistingData();
      
      // Phase 2: Test viewers watching ads at scale
      await this.testViewersWatchingAdsAtScale();
      
      // Phase 3: Generate comprehensive report
      await this.generateReport();
      
      console.log('🎉 Scaled ad viewing test completed!');
      
    } catch (error) {
      console.error('❌ Scaled ad viewing test failed:', error);
      process.exit(1);
    } finally {
  ...
  async testViewersWatchingAdsAtScale() {
    console.log('\n👀 PHASE 2: TESTING VIEWERS WATCHING ADS AT SCALE');
    console.log('-'.repeat(40));
    
    // Test with all available viewers
    const testViewers = this.viewers;
    console.log(`📊 Testing with ${testViewers.length} viewers for scaled testing`);
    
    const batchSize = 5; // Process 5 viewers at a time
    const totalBatches = Math.ceil(testViewers.length / batchSize);
    
    for (let batch = 0; batch < totalBatches; batch++) {
      const batchStart = batch * batchSize;
      const batchEnd = Math.min(batchStart + batchSize, testViewers.length);
      const batchViewers = testViewers.slice(batchStart, batchEnd);
      ...
      const batchPromises = batchViewers.map(viewer => 
        this.testSingleViewerWatchingAds(viewer)
      );
  ...
  async testSingleViewerWatchingAds(viewer) {
    try {
      ...
      // Watch 1-2 ads per viewer (reduced to avoid overwhelming system)
      ...
      for (let i = 0; i < adsToWatch; i++) {
        try {
          ...
          // Complete watching
          const completeResult = await this.testUtils.completeWatchingAd(viewer.phone, randomAd.id, proofToken);
  ...
  async generateReport() {
    ...
    console.log('\n🎯 SCALED AD VIEWING TEST RESULTS');
    ...
    const adViewSuccessRate = this.testResults.adsViewed > 0 
      ? (this.testResults.adsViewed / (this.testResults.viewersProcessed * 1.5) * 100).toFixed(2)
      : 0;
    ...
    if (adViewSuccessRate < 50) {
      console.log('   ⚠️ Ad viewing success rate is below 50%. Check ad completion API.');
    }
    if (this.testResults.errors.length > 20) {
      console.log('   ⚠️ High error rate detected. Review error handling and system stability.');
    }
    
    if (viewerSuccessRate >= 90 && adViewSuccessRate >= 50) {
      console.log('   ✅ Good performance! System is ready for production-scale ad viewing.');
    }
  ...
  async saveDetailedReport() {
    const report = {
      ...
      testType: 'Scaled Ad Viewing Test',
      summary: {
    ...
    const fs = require('fs');
    const reportPath = `./scaled-ad-viewing-test-report-${Date.now()}.json`;
  ...
  generateRecommendations() {
    ...
    const viewerSuccessRate = this.testResults.viewersProcessed > 0 
      ...
      : 0;
    const adViewSuccessRate = this.testResults.adsViewed > 0 
      ? (this.testResults.adsViewed / (this.testResults.viewersProcessed * 1.5) * 100)
      : 0;
    ...
    if (adViewSuccessRate < 50) {
      recommendations.push('Investigate and fix ad completion API 500 errors');
    }
    if (this.testResults.errors.length > 20) {
    ...
    if (viewerSuccessRate >= 90 && adViewSuccessRate >= 50) {
      recommendations.push('System is performing well - ready for production');
    }
...
if (require.main === module) {
  const test = new ScaledAdViewingTest();
  test.run().catch(error => {
    console.error('❌ Failed to run scaled ad viewing test:', error);
    process.exit(1);
  });
}

module.exports = ScaledAdViewingTest;
