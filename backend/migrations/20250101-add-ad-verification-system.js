...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const adsColumns = await queryInterface.describeTable('ads');
    
    // Add verification fields to existing ads table only if they don't exist
    if (!adsColumns.verification_status) {
    ...
    if (!adsColumns.rejection_reason) {
    ...
    if (!adsColumns.submitted_for_review_at) {
    ...
    // Check if ad_appeals table already exists
    const tables = await queryInterface.showAllTables();
    
    // Create ad_appeals table only if it doesn't exist
    if (!tables.includes('ad_appeals')) {
    ...
    // Create ad_verification_history table only if it doesn't exist
    if (!tables.includes('ad_verification_history')) {
