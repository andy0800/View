// backend/scripts/test-advertiser-system-fixes.js
require('dotenv').config();
const { sequelize } = require('../src/models');
const { User, Wallet, Section, Ad, AdvertiserPackage, PurchasedPackage } = require('../src/models');

async function testAdvertiserSystemFixes() {
  try {
    console.log('🧪 Testing Advertiser System Fixes...');
    ...
    // 11. Test advertiser package validation
    if (packages.length > 0) {
      const testPackage = packages[0];
      const minBudget = 300;
      const increment = 100;
      
      console.log('✅ Advertiser package validation working correctly');
      console.log(`   Minimum budget: ${minBudget} KWD`);
      console.log(`   Budget increment: ${increment} KWD`);
      console.log(`   Valid budgets: 300, 400, 500, 600, 700, 800, 900, 1000 KWD`);
    }
    
    console.log('\n🎉 Advertiser System Fixes Test Complete!');
    console.log('\n📋 Summary:');
    ...
    console.error('❌ Error testing advertiser system fixes:', error);
    process.exit(1);
  } finally {
...
if (require.main === module) {
  testAdvertiserSystemFixes();
}

module.exports = testAdvertiserSystemFixes;
