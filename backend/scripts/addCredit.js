...
(async () => {
  try {
    ...
    // 8) Credit the wallet and persist (convert KWD to fils for storage)
    const amountInFils = validateAndConvertToFils(amount);
    wallet.balance = (wallet.balance || 0) + amountInFils;
    await wallet.save();

    console.log(`💰 New balance: ${filsToKwd(wallet.balance)} KWD (${wallet.balance} fils)`);
    process.exit(0);
