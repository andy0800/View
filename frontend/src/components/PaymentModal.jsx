...
const steps = ['Payment Details', 'Payment Processing', 'Payment Complete'];
...
export default function PaymentModal({ open, onClose, onSuccess }) {
  ...
  const validateForm = () => {
    const validation = paymentService.validatePaymentForm(formData);
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      ...
      const session = await paymentService.createPaymentSession({
        amountKWD: parseFloat(formData.amount),
  ...
  const handlePaymentProcessing = async (sessionId) => {
    try {
      ...
      if (process.env.NODE_ENV === 'development' || import.meta.env.VITE_PAYMENT_SIMULATION === 'true') {
        await paymentService.simulatePayment(sessionId, formData.amount);
      }
      ...
      if (verification.status === 'completed') {
        ...
        setSuccess(`Payment of ${formData.amount} KWD completed successfully!`);
        setActiveStep(2);
        ...
        return (
          <Box sx={{ mt: 2 }}>
            ...
            <TextField
              ...
              InputProps={{
                startAdornment: <InputAdornment position="start">KWD</InputAdornment>,
              }}
              sx={{ mb: 2 }}
            />
          ...
          <Box sx={{ textAlign: 'center', py: 4 }}>
            {paymentStatus === 'success' ? (
              <>
                ...
                <Typography variant="body2" color="textSecondary" sx={{ mb: 2 }}>
                  Your account has been credited with {formData.amount} KWD
                </Typography>
                <Box sx={{ mt: 2 }}>
                  <Chip 
                    icon={<TrendingUp />}
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        <Box display="flex" alignItems="center" gap={1}>
          ...
          <Typography variant="h6">Add Credit to Account</Typography>
        </Box>
      </DialogTitle>
